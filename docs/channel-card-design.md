...
```go
// serving.Decorator 应用于每条出站消息(占位/过程更新/终稿/问句/错误),
...
type Outbound struct {
    ...
    Skip     bool           // 装饰器置 true = 本步不发送(dispatcher 消费,不出适配器)
}
```
...
**processing 与 reply_mode 的关系**:生命周期语义**全模式统一产生**
——不论 text/card/stream,dispatcher 都按 processing 首发 → 过程更新
→ 终稿收口驱动,每一步都送进装饰器;**发不发由第三方定**(装饰器置
`Skip` 否决该步发送,见 §3 签名)。reply_mode 降格为**无装饰器时的
内置默认策略**:text = 跳过全部 processing(现行为)、card = 占位+
原地更新、stream = 占位+增量刷新。机制细节:某步被 Skip 则没有占位
msgID,后续阶段自动用 Send 而非 Update(dispatcher 机械处理,装饰器
不用关心)。进度事件流的发射同样与模式无关,on_progress 任何模式都
收到事件。登记:stream 的 processing.Text 是半成品正文而非占位文案,
装饰器需要区分时再补显式信号(如 Partial bool),做 stream 适配时定。
...
3. **Native/Skip 框架永远不置**:它们是装饰器专属的输出位。

Skip 的边界:对 processing/过程更新随意否决(这正是它的主用途);对
answer/question 同样生效(第三方可能已经自行送达,框架不越权),但
记 warn 日志——终稿被吞而用户无感知是事故形态,日志留证。
