...
**processing 与 reply_mode 的关系**:框架产生的 processing 消息只在
card(占位+过程更新)与 stream(占位+增量刷新,Text 为不断变长的
半成品正文)出现;text 模式一锤定音,没有可更新载体,直接 answer。
但**进度事件流的发射与 reply_mode 解耦**——on_progress 在任何模式下
都收到事件(text + on_progress 合法:第三方自行决定是否发过程消息)。
登记:stream 的 processing.Text 是半成品正文而非占位文案,装饰器需要
区分时再补显式信号(如 Partial bool),做 stream 适配时定。
