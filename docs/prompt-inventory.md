...
| D. 引擎提示词 | ★★★★ | 输出格式 JSON schema 内联+规则列表(rewoo 含并行判据与 {eN} 引用语法);solver 有"证据可能含失败,如实反映"防编造条款。短板:未随模型对照迭代过(冒烟验证级) |
...
| # | 项 | 类 | 预期收益 |
|---|---|---|---|
| P1 | todo_write 描述补 2-3 个带 reasoning 的正反例(Claude Code 模式) | C | 弱模型计划触发率(已知差距,例子比规则管用) |
| P2 | exec 默认描述补使用边界(适用/不适用场景一句) | C | 减少脚本工具被误用为万能出口 |
| P3 | 遗留计划文案压缩(~⅓) | B | 每步注入的 token 成本 |
| P4 | D 类引擎提示词做一轮真机对照(现为冒烟验证级) | D | 编排引擎在弱模型下的格式遵循 |
| ⚪ | 英文空头承诺表随英文输出占比扩充 | F | 观察项,出现漏拦再加 |
