...
step 联动(定稿形态,提示词与参数彻底拆分):model 步骤的提示词写
`prompt:`(标量,前缀识别引用),`args` 一律只放参数——model 步骤为
绑定映射(键=模板占位符,不存在报错),工具/component 步骤为标量模板
或参数映射(映射拼 JSON 对象):

```yaml
- name: fetch
  use: "tools/cms/get_doc"
  args: {doc_id: policy-2026}                                   # 参数映射拼 JSON
- name: answer
  use: model
  prompt: "cap://prompt/fornax/oec.fulfillment.insight_mask"    # 提示词(引用或字面量)
  args: {analysis: "{fetch}", tone: formal}                     # 纯参数绑定
```

违规形态全部报错指路:model 缺 prompt、工具步骤带 prompt、args 里写
提示词引用/保留键、绑定键无对应占位符。
...
| `input` | 步骤级输入(graph/workflow steps 的字段;模板在调用方作用域渲染,成为被调组件的用户消息并重设其 `{$input}`) |
| `deliver` | 交付语义(skill/component 级):缺省=证据(供大脑合成);`attach` 存底、终答引用 `#dN` 即原文随行;`always` 恒随行;`direct` 独占轮次时原文即终答 |
| `sources` | 能力供给源声明(顶层与 namespace 同构) |
| `tools` | 工具面(引用集合或其白名单收紧) |
| `params` | 形参声明(对外接口 schema) |
| `args` | 实参(标量模板或参数映射;绝不承载提示词) |
| `prompt` | 提示词(字面量或 cap://prompt/ 前缀引用;model 步骤专属键) |
