...
**window 与 keep_recent(机制解耦 + 一条配置建议)**:
视图 = `[滚动摘要] + [首条用户消息锚定] + 近期原文`。窗口裁剪**始终保留
合成头部**(摘要 + 锚定),只裁近期原文——`session.window` 小于
`loop.compaction.keep_recent` 时摘要也不会被裁掉(见 agent.windowKeepingHead)。
两个旋钮职责不同:`window` = 织入 prompt 的近期原文条数,`keep_recent` =
压缩时保留不摘要的近期条数。这是**机制层面**的解耦,不是硬约束。

> **配置建议:`session.window` > `loop.compaction.keep_recent` + 2。**

摘要边界在 `len−keep_recent`(之前的压成梗概),窗口边界在 `len−window`。
若 `window < keep_recent`,`[len−keep_recent, len−window]` 这段"夹层"消息
既**不在摘要**(摘要只覆盖 keep_recent 之前)、也**不在窗口**——只能靠
`session.recall` 按词法/近因部分捞回,不进直接上下文。取 `window > keep_recent`
则窗口外的一切都被摘要覆盖,prompt = 摘要 + 窗口严丝合缝、无夹层。框架
两种都能跑(夹层不报错、由 recall 兜底),但无夹层是更完整的默认。
