...
用法:agent 层 `stores:` 声明具名实例 `{name, kind, type, config, ttl}`,
模块块的 store 槽用 `cap://store/<kind>/<name>` 引用;换后端=改 type、
或指向另一实例;跨 agent 共享=各自指同一实例。裸 `store: file` 作缺省
简写,存量零迁移。redis 后端空导入 [provider/redisstore](provider/redisstore)
即启用(session 与 KV 家族一并覆盖)。
...
todo_store: cap://store/todo/plans
memory: {window: 40, store: cap://store/session/sess}
context_hygiene: {digest_over: 4000, store: cap://store/result/cache}
```
