...
**window 与 keep_recent 相互独立(无隐性约束)**:
视图 = `[滚动摘要] + [首条用户消息锚定] + 近期原文`。窗口裁剪**始终保留
合成头部**(摘要 + 锚定),只裁近期原文——所以 `session.window` 可以小于
`loop.compaction.keep_recent`,摘要也不会被裁掉(见 agent.windowKeepingHead)。
两个旋钮各管各的:`window` = 织入 prompt 的近期原文条数,`keep_recent` =
压缩后保留的近期条数,不需要满足任何大小关系。
