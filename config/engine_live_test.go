...
func engineLiveProviders(t *testing.T) map[string]func(ctx context.Context) (einomodel.ToolCallingChatModel, error) {
	...
	if key := os.Getenv("MINIMAX_API_KEY"); key != "" {
		...
		out["minimax/MiniMax-Text-01"] = func(ctx context.Context) (einomodel.ToolCallingChatModel, error) {
			m, err := model.Build(ctx, "minimax", map[string]any{"api_key": key, "base_url": base})
			if err != nil {
				return nil, err
			}
			return loop.RetryModel(m, retry), nil
